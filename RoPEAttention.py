...
class RoPEAttention(tf.keras.Model):
    ...
    def call(self, x, return_attn_weights=False):
        ...
        q_out = tf.transpose(tf.matmul(q_t, self.R),perm=[0, 1, 2])  # Transpose back to (batch_size, seq_length, r_dim)
        k_t = tf.transpose(k, perm=[1, 0, 2])  # Transpose q from (batch_size, seq_length, q_dim) to (batch_size, q_dim, seq_length)
        k_out = tf.transpose(tf.matmul(k_t, self.R),perm=[0, 1, 2])  # Transpose back to (batch_size, seq_length, r_dim)
        v_t = tf.transpose(v, perm=[1, 0, 2])  # Transpose q from (batch_size, seq_length, q_dim) to (batch_size, q_dim, seq_length)
        v_out = tf.transpose(tf.matmul(v_t, self.R),perm=[0, 1, 2])  # Transpose back to (batch_size, seq_length, r_dim)
